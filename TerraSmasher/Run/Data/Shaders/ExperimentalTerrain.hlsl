...
struct ExperimentalTerrainRenderResource
{
    ...
    uint sharedVertexBufferIndex; 
    uint perVertexBufferIndex; 
};
...
float2 UnpackNormal(uint packedNormal)
{
    float x = float((packedNormal >> 0) & 0xFF) / 255.f;
    float y = float((packedNormal >> 8) & 0xFF) / 255.f;
...
ConstantBuffer<ExperimentalTerrainRenderResource> renderResources : register(b0);
//----------------------------------------------------------------------------------------------------
v2p_t VertexMain(uint vertexID : SV_VertexID)
{
    ...
    StructuredBuffer<PerVertex> perVertexBuffer = ResourceDescriptorHeap[renderResources.perVertexBufferIndex];
    StructuredBuffer<SharedVertex> sharedVertexBuffer = ResourceDescriptorHeap[renderResources.sharedVertexBufferIndex];

    PerVertex perVertexData = perVertexBuffer[vertexID];
    SharedVertex sharedVertex = sharedVertexBuffer[perVertexData.sharedVertexIndex];

    vs_input_t input;
    input.modelPosition = sharedVertex.position;
    input.encodedSmoothNormal = UnpackNormal(sharedVertex.packedNormal);
    input.materialID1 = (perVertexData.packedMaterial >> 0) & 0xFF;
    input.materialID2 = (perVertexData.packedMaterial >> 8) & 0xFF;
    input.blendFactor = float((perVertexData.packedMaterial >> 16) & 0xFF) / 255.f;

#include "Game/DualContouring2.hpp"
...
DualContouring2::DualContouring2()
{
...
void DualContouring2::RegenerateMesh(IntGrid3 const& grid, std::vector<float> const& sdfValues, float blockyBlendFactor /*= 0.f*/)
{
	m_isReady = false;

	m_grid = grid; // same size with sdfValues
	m_sdfValues = sdfValues;
	m_blockyBlendValue = blockyBlendFactor;

	RecalculateCornerGradient();
	ResetData();
...
void DualContouring2::RecalculateCornerGradient()
{
	...
	m_cornerGradients.clear();
	m_cornerGradients.resize(m_sdfValues.size());

	for (int z = 1; z < m_grid.GetSizeZ() - 1; ++z)
	{
		for (int y = 1; y < m_grid.GetSizeY() - 1; ++y)
		{
			for (int x = 1; x < m_grid.GetSizeX() - 1; ++x)
			{
				IntVec3 currentIndex3D(x, y, z);
				int currentIndex1D = m_grid.Linearize(currentIndex3D);

				Vec3 gradient = Vec3(GetDensity(currentIndex1D + strideAxisX) - GetDensity(currentIndex1D - strideAxisX),
					GetDensity(currentIndex1D + strideAxisY) - GetDensity(currentIndex1D - strideAxisY),
					GetDensity(currentIndex1D + strideAxisZ) - GetDensity(currentIndex1D - strideAxisZ));

				m_cornerGradients[currentIndex1D] = gradient.GetNormalized();
			}
		}
	}
...
void DualContouring2::EstimateIsosurfaceVertexAndNormal()
{
	// linear transversal of lattices
	for (int z = 1; z < m_grid.GetSizeZ() - 2; ++z)
	{
		for (int y = 1; y < m_grid.GetSizeY() - 2; ++y)
		{
			for (int x = 1; x < m_grid.GetSizeX() - 2; ++x)
			{
				...
				{
					Vec3 cornerGradients[Isosurface::kCubeCornerCount] = {};

					for (int i = 0; i < Isosurface::kCubeCornerCount; ++i)
					{
						int cornerIndex1D = m_grid.Linearize(minCornerIndex3D + Isosurface::kCubeCornerOffsets[i]);
						cornerGradients[i] = m_cornerGradients[cornerIndex1D];
					}


					//Vec3 centroid = GetEdgeIntersectionsCentroid(cubeIndex, cornerDensities, m_isosurfaceValue);
					Vec3 isoPoint = GetVertexFromHermiteData(cubeIndex, cornerDensities, cornerGradients, m_isosurfaceValue);
					...
					m_vertices.push_back(Vertex_Terrain(position, normal));
...
void DualContouring2::BuildMeshQuads()
{
	int strideAxisX = m_grid.Linearize(IntVec3(1, 0, 0));
	int strideAxisY = m_grid.Linearize(IntVec3(0, 1, 0));
	int strideAxisZ = m_grid.Linearize(IntVec3(0, 0, 1));
	...
	for (auto const& item : m_isosurfaceVertexIndexs)
	{
		...
		// No need to check maximum? it is not generated
		if (index3D.x < 2 || index3D.y < 2 || index3D.z < 2)
		{
			continue;
		}
...
void DualContouring2::TryBuildQuad(int p1, int p2, int strideAxisB, int strideAxisC)
{
	//int p1 = index1D;
...
Vec3 DualContouring2::GetVertexFromHermiteData(int cubeIndex, float* cornerDensities, Vec3* cornerGradients, float isosurfaceValue)
{
	...
	intersectionPointNormals.reserve(Isosurface::kCubeEdgeCount);
	...
	for (int edge = 0; edge < Isosurface::kCubeEdgeCount; ++edge)
	{
		if (edgeFlags & (1 << edge))
		{
			...
			Vec3 gradient0 = cornerGradients[v0];
			Vec3 gradient1 = cornerGradients[v1];
			...
			Vec3 n = Interpolate(gradient0, gradient1, t).GetNormalized();
			intersectionPointPositions.push_back(p);
			intersectionPointNormals.push_back(n);
		}
	}
	...
	Vec3 solvedPos;
	/*float error = */qef_solve_from_points_3d(
		&intersectionPointPositions[0].x,
		&intersectionPointNormals[0].x,
		(int)intersectionPointPositions.size(), 
		&solvedPos.x
	);
	AABB3 box = AABB3(Vec3(0.f, 0.f, 0.f), Vec3(1.f, 1.f, 1.f));
	solvedPos = box.GetNearestPoint(solvedPos);
	return solvedPos;


	// QEF method // float precision is not enough, use double or QR decomposition
	//Mat44 AtA = Mat44(Vec4(), Vec4(), Vec4(), Vec4());
	//Vec3 AtB = Vec3();
	...
	//	float edgePosDistance = DotProduct3D(edgeNormal, edgePos);

	//	AtB += edgeNormal * edgePosDistance;

	//	AtA = AtA + Mat44(
	//		Vec4(edgeNormal.x * edgeNormal.x, edgeNormal.x * edgeNormal.y, edgeNormal.x * edgeNormal.z, 0.0f),
	//		Vec4(edgeNormal.y * edgeNormal.x, edgeNormal.y * edgeNormal.y, edgeNormal.y * edgeNormal.z, 0.0f),
	//		Vec4(edgeNormal.z * edgeNormal.x, edgeNormal.z * edgeNormal.y, edgeNormal.z * edgeNormal.z, 0.0f),
	...
	//
	//AtA.Inverse();
	//Vec3 result = AtA.TransformVectorQuantity3D(AtB);

	//return result;

	//AABB3 box = AABB3(Vec3(0.f, 0.f, 0.f), Vec3(1.f, 1.f, 1.f));
	//result = box.GetNearestPoint(result);
	...
	//constexpr float THRESHOLD = 0.00000001f;
...
STATIC Vec3 DualContouring2::GetEdgeIntersectionsCentroid(int cubeIndex, float* cornerDensities, float isosurfaceValue)
{
	if (cubeIndex == 0 || cubeIndex == 255)
	{
		return Vec3(0.5f, 0.5f, 0.5f); // center of the cube
...
STATIC Vec3 DualContouring2::GetSdfGradient(Vec3 const& s, float* d)
{
